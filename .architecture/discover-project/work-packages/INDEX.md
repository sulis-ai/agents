...
| WP-002 | Author derive-consumer-tenant Tool + Python implementation + fixed vectors | create | backend | done | WP-001 | WP-006 | 3k / 3k | Form #8 (`tenant.py`); ADR-002 |
| WP-003 | Implement Detect phase — RepoInspector port + LocalFilesystemInspector adapter | create | backend | done | WP-001 | WP-008 | 4k / 4k | Form Ports #1; Armor External deps (git CLI) |
| WP-004 | Implement Infer phase — ConfigurationInferrer port + LLM and Null adapters + token budget | create | backend | done | WP-001 | WP-008 | 4k / 4k | Form Ports #2; Armor External deps (LLM); ADR-006 |
...
| WP-006 | Implement Mint phase — atomic write + path safety + signal handler + slug derivation | create | backend | done | WP-001, WP-002 | WP-008 | 4k / 4k | Armor Atomic write semantics; Form §Slug derivation |
