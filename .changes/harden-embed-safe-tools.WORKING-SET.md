...
- **Q5 (founder-raised):** Enumerate the *full* current write-surface the agent legitimately needs beyond the worktree — confirmed so far: the resolved **brain dir** (rw, shared), this change's **state dir** under `~/.sulis/changes/{id}/` (rw, change-scoped), git-common-dir, tools/cache (ro). Are there others today (cockpit cache? changeset/release artifacts? captured-lessons paths?)? The resolver must enumerate them explicitly — and stay narrowest-root, so `~/.sulis/changes/{OTHER}/` is never writable from this change.
