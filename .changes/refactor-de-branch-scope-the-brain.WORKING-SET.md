...
- 2026-06-13T12:49:14Z — REVIEW done (focused, proportionate — no sensitive surface). 7-tier check: build OK (compileall+manifests), security n/a (no auth/net/user-input/secrets; path resolution unchanged in shape, no untrusted input), tests green test-first, reliability preserved (best-effort fallbacks), readable, maintainable (reuses sulis_state_base single source of truth). VERDICT: good to ship. Honest note for founder: deliberate behaviour change to the default location — downstream users with an old in-repo .brain are not auto-migrated (migration was a spec non-goal); Sulis itself protected via contract pin.
- 2026-06-13T12:57:46Z — FOUNDER Q: test contamination of real brain. Verified NO leak (no test ULID in ~/.sulis/.brain). CI inherently safe (ephemeral home). Found: brain now rides the SAME SULIS_STATE_DIR isolation as change-state, so existing rootdir conftest fixture already walls off the brain for the brain-touching suite. analyse-codebase suite has no isolation but never touches the brain (no leak path). Added belt-and-braces: session-start _brain_real_home_floor in tests/conftest.py (sets SULIS_STATE_DIR tmp at session start if unset) + documented brain isolation rides on _isolate_sulis_state. 2925 unit green.
