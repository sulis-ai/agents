...
Editable fields here are `name` (always) and, optionally, `branch_policy`
(an enum the schema constrains). Every other schema-required field is
preserved — critically `source` and `belongs_to_product_ref`, which the
Settings tree depends on. Reject-on-invalid is unchanged: an edit that would
break the schema writes nothing and exits non-zero.

Usage:
    edit-project.py --scripts-dir <dir> --base-dir <brain/instances>
                    --id <dna:project:..> --name <new-name>
...
def main() -> int:
    ...
    parser.add_argument("--name", required=True, help="the new project name")
