...
| `GET /api/changes/:id/transcript`    | Chronologically-merged chat messages — read from the cockpit's own durable thread record first, falling back to the Claude session transcripts when no durable record exists yet.                                                                                    | `TranscriptMessage[]`                                   |
