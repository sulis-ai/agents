...
description: >
  ...
  weak-answer drill list. Agent-proposed at the end of /investor-coach:build-deck.
user_invocable: true
---
...
The agent proposes this skill at the end of `/investor-coach:build-deck`. The
founder may decline; rehearsal is optional but strongly recommended.

## When to invoke

- Right after `/investor-coach:build-deck` is complete and the founder accepts the
  rehearsal proposal.
...
If significant revisions are needed, return to `/investor-coach:narrative`.
If only drill is needed, schedule the next rehearsal date in the
journal.

If the founder is ready for the actual meeting, propose
`/investor-coach:validate` as the final pre-flight check.
...
- [ ] Next step proposed (`/investor-coach:narrative` if revisions needed, else
      `/investor-coach:validate`)
