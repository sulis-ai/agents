...
- After `/investor-coach:narrative` is complete and slides have proof-point
  references.
...
`/investor-coach:build-deck` (after addressed items are reflected in slides).
...
- [ ] Next step (`/investor-coach:build-deck`) proposed (after revisions if any)
