...
- If something exists: invoke `/investor-coach:brand-discovery` to extract per BR-01–BR-06.
- If nothing exists: invoke `/investor-coach:brand-discovery` to propose per BP-01–BP-NN.
...
### Phase 4: Market Research (skill: `/investor-coach:market-research`)
...
**Run mode:** Skill-driven. You propose `/investor-coach:market-research`; the
...
### Phase 5: Financial Modelling (skill: `/investor-coach:financial-model`)

**Purpose:** Produce a defensible, stage-appropriate financial model
grounded in the proof-points from Phase 4.

**Run mode:** Skill-driven. You propose `/investor-coach:financial-model`; the
...
### Phase 6: Narrative Synthesis (skill: `/investor-coach:narrative`)
...
### Phase 7: Adversarial Sweep (skill: `/investor-coach:adversarial-review`) — **HAT CHANGE**
...
### Phase 8: Design & Build (skill: `/investor-coach:build-deck`)
...
### Phase 9: Rehearsal (skill: `/investor-coach:rehearsal`) — **Agent-proposed**
...
`/investor-coach:build-deck` completes, say:
...
If the founder accepts, invoke `/investor-coach:rehearsal`. If they decline,
note it in the journal and offer `/investor-coach:validate` as a final check.
...
`investor-coach` is self-contained. It does **not** require any other Sulis AI
...
4. Determine the current phase by checking which artifacts exist:
   ...
   - BRAND.md but no `02-research/MARKET_RESEARCH.md` → propose `/investor-coach:market-research`
   - `02-research/MARKET_RESEARCH.md` PASS but no `03-financials/MODEL.yaml` → propose `/investor-coach:financial-model`
   - `03-financials/MODEL.yaml` but no `04-narrative/NARRATIVE.md` → propose `/investor-coach:narrative`
   - `04-narrative/NARRATIVE.md` but no `05-adversarial/ADVERSARIAL_REVIEW.md` → propose `/investor-coach:adversarial-review`
   - `05-adversarial/ADVERSARIAL_REVIEW.md` but no `04-narrative/DECK.pptx` → propose `/investor-coach:build-deck`
   - `04-narrative/DECK.pptx` but no `06-verification/REHEARSAL_NOTES.md` → propose `/investor-coach:rehearsal`
...
A deck is "complete" when `/investor-coach:validate` returns PASS. Until then, it
is in progress.
