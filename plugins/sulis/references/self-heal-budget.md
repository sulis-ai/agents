...
| **Code-review inline fix loop** (v0.20.1+) | 6.5 | 3 | `/sulis:code-review` produces findings → inline fix → re-run `/code-review` → loop. Most fixes converge in 1-2 cycles; the 3rd is a safety margin before switching to Path B (auto-draft remediation WP) or Path C (founder-flagged exception via BLOCKER). On 3rd iteration with findings still remaining: stop the loop and escalate. Do NOT loop indefinitely — the gate fails closed via BLOCKER `trigger=code-review-unaddressed` if neither remediation nor exception path resolves the residue. |
