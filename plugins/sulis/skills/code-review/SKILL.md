...
1. A self-contained review bundle at `.architecture/{project}/code-reviews/PR-{number}-{YYYY-MM-DDTHHMMSSZ}/` containing:
   ...
   security | quality`. `/sulis:harden-codebase` discovers them recursively under
...
*"recommend full `/sulis:codebase-audit` to size the broader gap"* — don't
list every site.
...
self-attestation. This is what `/sulis:harden-codebase` and future `/code-review`
runs read.
...
>
> Full review at `.architecture/{project}/code-reviews/PR-{number}-{YYYY-MM-DDTHHMMSSZ}/REVIEW.md`. {If draft fixes were queued: "N draft fixes are ready in the review's `hardening-deltas/` folder — run `/sulis:harden-codebase` to apply them when you're ready."} {If PH-01 or PH-02 fired high: "Your PR is large enough or mixed enough that splitting would help — run `/sulis:suggest-split {PR-number}` to get a specific proposal with the git commands."}
...
| **Tier 2 — Technical detail** | Engineers reviewing the PR, downstream agents (`/sulis:harden-codebase`, future runs of `/code-review`) | Internal taxonomy intact (CR-NN, PH-NN, etc.), signal tables, self-attestation, full evidence trail. |
...
review. `/sulis:harden-codebase` finds the deltas because it scans
`.architecture/{project}/` recursively for `HD-*.md`. Drafts ship at
`status: proposed`; promoting to `status: accepted` in the delta's
frontmatter is what tells `/sulis:harden-codebase` to implement them.
...
- **What to do** — specific next step. If a fix pattern exists in the codebase, point to it by file path. If a draft fix is queued, mention it can be applied with `/sulis:harden-codebase`.
...
**What to do:** When the response comes back from the API, pull `hasMore` out alongside `codes` and `total`. The pattern is on line 113 — you've already declared the shape, you just need to use it: `const { data, total, hasMore } = body`. A draft fix is queued — run `/sulis:harden-codebase HD-019` to apply it.
...
**If this sounds unfamiliar, run `/sulis:suggest-split {PR-number}`** — it reads the changes, proposes a specific split for *this* pull request, and gives you the exact git commands for each piece. You can copy and run them yourself, or share them with a developer.
...
- **Deep** ("this PR touches load-bearing code") — Standard + no neighbour cap, recommend full `/sulis:codebase-audit` afterwards.
...
- **With `/sulis:harden-codebase`** — draft fixes produced here are handed to
  `/sulis:harden-codebase` if accepted. Provenance preserved via
  ...
  order).
- **With `/sulis:codebase-audit`** — when neighbour-ring patterns suggest a
