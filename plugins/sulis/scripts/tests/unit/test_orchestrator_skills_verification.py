...
def test_specify_exists() -> None:
    assert _SPECIFY.exists(), f"specify SKILL.md missing at {_SPECIFY}."
...
def test_req_val_exists() -> None:
    assert _REQ_VAL.exists(), f"requirements-validation SKILL.md missing at {_REQ_VAL}."
...
def test_req_val_says_p_ver_fail_means_fail(req_val_text: str) -> None:
    ...
    has_verdict_term = "GAPS_FOUND" in req_val_text or "FAIL" in req_val_text
...
def test_req_tpl_exists() -> None:
    assert _REQ_TPL.exists(), f"requirements-templates SKILL.md missing at {_REQ_TPL}."
