...
# ---------------------------------------------------------------------------
# WP-002 — platform-contracts/ storage directory + derived INDEX.md
# ---------------------------------------------------------------------------
_CONTRACTS_DIR = (
    _REPO_ROOT / "plugins" / "sulis" / "references" / "platform-contracts"
)
_CONTRACTS_INDEX = _CONTRACTS_DIR / "INDEX.md"

# The derived-view column headers the reuse + freshness path reads
# (FR-011 / FR-013). Matched as substrings so header punctuation is tolerated.
_INDEX_REQUIRED_HEADERS = (
    "Platform",
    "Contract",
    "Harness run",
    "retrieval-date",
...
def test_storage_and_index_present() -> None:
    """WP-002: the platform-contracts/ directory + its derived INDEX.md exist,
    and the INDEX carries the freshness-column header (FR-010 / FR-011 /
    FR-013; ADR-002)."""
    assert _CONTRACTS_DIR.is_dir(), (
        f"Expected the storage directory at {_CONTRACTS_DIR} (ADR-002)."
    )
    assert _CONTRACTS_INDEX.is_file(), (
        f"Expected a derived INDEX.md at {_CONTRACTS_INDEX}."
    )
    index_text = _CONTRACTS_INDEX.read_text(encoding="utf-8")
    missing = [h for h in _INDEX_REQUIRED_HEADERS if h not in index_text]
    assert not missing, (
        "platform-contracts/INDEX.md missing required column "
        f"header(s): {missing}."
    )
