...
| **Interface contract / ServiceSpec artifact** | The design-time AGREEMENT that defines a tool/API/SDK seam: per operation, the schema layer (operations, input/output types, three-category errors per CONTRACT_FIRST_STANDARD CF-03) PLUS the CF-10 founder-reviewable dimensions (auth/permissions, audience, plain-language user guide, error fixes). It is a mandatory section of the comprehensive design document's Solution Design (FR-18), authored first for cross-kind seams, and the source of the operations the tool-surface walk classifies (FR-19). Where the project already produces ServiceSpecs (`architecture/SERVICE_SPECIFICATION.md`), the ServiceSpec IS the contract. This is the AGREEMENT (what the seam is) — distinct from the ServiceSpec *binding*, which is the WIRING (that a concrete handler implements it). |
| **ServiceSpec binding** | The declaration that WIRES an exposed tool/operation to its concrete handler — the implementation side of an interface contract operation. A tool that serves an interface but has no ServiceSpec binding is the canonical "looks-built-but-isn't-wired" failure. Distinct from the *interface contract* (the agreement): the contract says the operation should exist and what it promises; the binding proves a handler is wired to deliver it. |
...
| Interface contract / ServiceSpec artifact | "the contract", "ServiceSpec", "the API contract", "the schema layer", "the interface" |
...
| **Interface contract / ServiceSpec artifact** | **ServiceSpec binding** | The contract is the AGREEMENT (what operations the seam offers, with their types, errors, and CF-10 founder-facing dimensions); the binding is the WIRING (that a concrete handler implements a contract operation). The contract is authored first (design-time); the binding is what the tool-surface walk cites to classify an operation EXISTS. A change can have a contract with no binding (planned-WP) — but a binding with no contract operation is a contract gap (FR-19, UC-04 2a). |
